...
### Overview of Support for the Full LDP Specification
...
- Only the ldp:BasicContainer is supported currently, with direct managed membership. This mirrors how LOD Gateway manages the Named Graphs uploaded to it.
- The Container hierarchy will mirror the URL path. eg /annotations/test/12345 will have the following container hierarchy:
    - root '/' -contains-> '/annotations/' -contains-> '/annotations/test/' -contains-> '/annotations/test/12345'
- A Container will contain either other ldp:BasicContainer resources, or named graphs (the JSON-LD documents). 
