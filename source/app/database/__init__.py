...
class Database:
	...
	def __init__(self, shared=True):
		"""Initializing our database handler class, including configuring the database connection and credential details, and if the connection will be shared or not."""
	...
	def connect(self, autocommit=False):
		...
		try:
			...
			debug(str(e), error=True, indent=4)
	...
	def disconnect(self, connection=None):
		...
		if(connection):
			if(connection in self.cursors):
				...
				debug("self.cursors[connection] length = %d" % len(self.cursors[connection]), level=2, indent=4)
			try:
				connection.close()
			except:
		...
		elif(isinstance(self.connections, list) and len(self.connections) > 0):
		...
		debug("self.connections length = %d" % len(self.connections), level=2, indent=4)
	
	def cursor(self, connection=None, factory=NamedTupleCursor):
		"""Provide support for obtaining a new database cursor of the desired type. The default is a NamedTupleCursor. Other Pyscopg2 supported cursor types may be specified via the factory named argument if desired."""
