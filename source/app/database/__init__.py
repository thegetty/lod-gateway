...
class Database:
    def __init__(self, shared=True):
        ...
        debug(
            "Database.__init__(shared: %s) configuration = %s"
            % (shared, self.configuration),
            level=2,
        )
    ...
    def connect(self, autocommit=False):
        ...
        debug(
            "Database.connect(autocommit: %s) service = %s" % (autocommit, service),
            level=2,
        )
        ...
        try:
            ...
            debug(
                "Database.connect() Failed to connect to the %s database!"
                % (self.configuration["database"]),
                error=True,
            )
    ...
    def disconnect(self, connection=None):
        ...
        if isinstance(connection, psycopg2.extensions.connection):
                ...
                for index, cursor in enumerate(self.cursors[connection]):
                    if isinstance(cursor, psycopg2.extensions.cursor):
                        try:
                            ...
                            debug(
                                "Database.disconnect() Failed to close cursor (%s) for the %s database!"
                                % (cursor, self.configuration["database"]),
                                error=True,
                            )
                            debug(e, error=True, indent=1)
            ...
            try:
                ...
                debug(
                    "Database.disconnect() Failed to close connection for the %s database!"
                    % (self.configuration["database"]),
                    error=True,
                )
                debug(e, error=True, indent=1)
            ...
            try:
                ...
                debug(
                    "Database.disconnect() The specified connection (%s) could not be found in the list of connections!"
                    % (connection),
                    error=True,
                )
    ...
    def cursor(self, connection=None, factory=NamedTupleCursor):
        ...
        debug(
            "Database.cursor(connection: %s, factory: %s) called..."
            % (connection, factory),
            level=1,
        )
        ...
        if isinstance(connection, psycopg2.extensions.connection):
            try:
                ...
                if isinstance(cursor, psycopg2.extensions.cursor):
                    debug(
                        "Database.cursor() Created cursor of type: %s" % (type(cursor)),
                        level=2,
                    )
                    ...
                    debug(
                        "Database.cursor() Failed to create a new cursor for the %s database!"
                        % (self.configuration["database"]),
                        error=True,
                    )
                    return None
            ...
            debug(
                "Database.cursor() Unable to create a new cusor as there is no valid connection to the %s database!"
                % (self.configuration["database"]),
                error=True,
            )
            return None
            ...
            if isinstance(cursor, psycopg2.extensions.cursor):
                ...
                if result:
                    if result[0]:
                        ...
                        debug(
                            "Database.information(connection: %s) The query result did not contain a 'connections' property!"
                            % (connection),
                            error=True,
                        )
                else:
                    debug(
                        "Database.information(connection: %s) A valid query result could not be obtained!"
                        % (connection),
                        error=True,
                    )
            else:
                debug(
                    "Database.information(connection: %s) Unable to obtain a new database connection cursor!"
                    % (connection),
                    error=True,
                )
        else:
            debug(
                "Database.information(connection: %s) Unable to obtain a new database connection!"
                % (connection),
                error=True,
            )
