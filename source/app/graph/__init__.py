...
class GraphStore:
	...
	def update(graph_id, namespace, json_data):
		try:
			...
			debug("GraphStore.update() problem found with record %s, error: %s" % (graph_name,str(e)), error=True)
			return False
	...
	def delete(graph_id, namespace):
		...
		res = requests.post(neptune_endpoint,data={'query':"SELECT (count(?s) as ?count) { GRAPH <"+graph_name+"> {?s ?p ?o}}"})
		count_json = json.loads(res.content)
		if int(count_json['results']['bindings'][0]['count']['value'])>0:
			# drop graph
			res = requests.post(neptune_endpoint,data={'update':"DROP GRAPH <"+graph_name+">"})
			if res.status_code==200:
				return True
			else:
				return False