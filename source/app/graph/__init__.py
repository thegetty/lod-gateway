...
class GraphStore:
	...
	@classmethod
	def endpoint(cls):
		if(not (isinstance(cls.configuration["host"], str) and len(cls.configuration["host"]) > 0)):
			raise RuntimeError("No valid Amazon Neptune instance hostname has been specified! Please review application configuration and try again!")
	...
	def query(cls, query):
		...
		if(not cls.enabled()):
			debug("GraphStore.query(query: %s) graph store access disabled via configuration, returning immediately..." % (query), level=2)
			return True
		...
		try:
			...
			if(response.status_code == 200):
				...
				debug("GraphStore.query() Failed! HTTP Response Status Code: %d" % (response.status_code), error=True)
				if(response.content):
	...
	def select(cls, entityURI):
		...
		if(not cls.enabled()):
			debug("GraphStore.select(URI: %s) graph store access disabled via configuration, returning immediately..." % (entityURI), level=2)
			return True
		...
		try:
			...
			response = requests.post(
				cls.endpoint(),
				data={
					"query": "SELECT (count(?s) as ?count) { GRAPH <" + graphName + "> {?s ?p ?o}}"
				},
			)
	...
	def update(cls, entityURI, jsonData):
		...
		if(not cls.enabled()):
			debug("GraphStore.update(URI: %s) graph store access disabled via configuration, returning immediately..." % (entityURI), level=2)
			return True
	...
	def delete(cls, entityURI):
		...
		if(not cls.enabled()):
			debug("GraphStore.delete(URI: %s) graph store access disabled via configuration, returning immediately..." % (entityURI), level=2)
			return True
