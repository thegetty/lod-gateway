...
class Model(ABC):
		...
		if(isinstance(relationships, dict) and len(relationships) > 0):
			if(name in relationships):
				...
				if(isinstance(relationship, dict)):
					...
					debug("%s.__setattr__(name: %s) Relationship is invalid! Should be defined as a dictionary!" % (self.__class__.__name__, name), error=True)
	...
	def hasOne(cls, field=None, model=None, to=None, alias=None, **kwargs):
		...
		if(name):
			if(name in cls._relationships):
				raise RuntimeError("Cannot initialize model relationship '%s' as a relationship with the same name already exists! Use aliases if necessary to distinguish the relationships." % (name))
			else:
	...
	def count(cls, *args, **kwargs):
		debug("%s.count(args: %s, kwargs: %s) called..." % (cls.__name__, args, kwargs), level=1)
	...
	def find(cls, *args, **kwargs):
		debug("%s.find(args: %s, kwargs: %s) called..." % (cls.__name__, args, kwargs), level=1)
					...
					if(attributes):
						if(field in attributes):
							...
							if(model):
								...
								if(instance):
									instance.__fireEvent(event="fetch", stage="after", **kwargs)
									
									records.append(instance)
								else:
									debug("%s.find() Failed to instantiate a new %s class instance!" % (cls.__name__, model), error=True)
							else:
								debug("%s.find() Failed to determine model class!" % (cls.__name__), error=True)
						else:
							debug("%s.find() Failed to find model ID field within attributes!" % (cls.__name__), error=True)
					else:
						debug("%s.find() Failed to obtain attributes from result!" % (cls.__name__), error=True)
			else:
				debug("%s.find() No results were found!" % (cls.__name__), error=True)
		else:
			debug("%s.find() The query could not be prepared!" % (cls.__name__), error=True)
		
		return records
	
	@classmethod
	def findFirst(cls, *args, **kwargs):
		debug("%s.findFirst(args: %s, kwargs: %s) called..." % (cls.__name__, args, kwargs), level=1)
		...
		if(args and len(args) == 1 and isinstance(args[0], int)):
			field = cls.getIDField()
			if(field):
				results = cls.find(sprintf("%s = :%s:" % (field, field)), bind={field: args[0]}, limit=1, **kwargs)
		else:
	...
	def findFirstOrCreateNewInstance(cls, *args, **kwargs):
		debug("%s.findFirstOrCreateNewInstance(args: %s, kwargs: %s) called..." % (cls.__name__, args, kwargs), level=1)
	...
	def create(self, **kwargs):
		...
		if(self.__fireEvent(event="create", stage="before", **kwargs)):
			...
			query = self.prepareQuery("create", id=self.id, params=attributes, self=self)
			if(query):
	...
	def update(self, **kwargs):
		...
		if(self.__fireEvent(event="update", stage="before", **kwargs)):
			...
			if(self.hasChanged() or ("force" in kwargs and kwargs["force"] == True)):
				...
				query = self.prepareQuery("update", id=self.id, params=attributes, self=self)
				if(query):
					result = self.performQuery(query)
					if(result):
						result = True
			else:
				debug("%s.update(%s) Apparently %s has not changed?" % (self.__class__.__name__, kwargs, self), level=1, error=True)
	...
	def changedFields(self):
		...
		if(isinstance(attributes, dict) and len(attributes) > 0):
			for attribute in attributes:
				debug("attribute = %s (snapshot: %s)" % (attribute, (attribute in self._snapshot)), indent=1, level=3)
				
				if((not (attribute in self._snapshot)) or (not (self._snapshot[attribute] == attributes[attribute]))):
					if(attribute in self._snapshot):
						debug("attribute = %s (snapshot: %s)" % (attributes[attribute], self._snapshot[attribute]), indent=2, level=3)
					
					if(isinstance(self._autopopulated, list) and attribute in self._autopopulated):
						debug("%s.changedFields() Ignoring changed attribute (%s) as it is an autopopulated field..." % (self.__class__.__name__, attribute), level=2)
					else:
						changed[attribute] = attributes[attribute]
		
		debug("%s.changedFields() changed = %s" % (self.__class__.__name__, changed), level=2)
	...
	def __fireEvent(self, event=None, stage=None, **kwargs):
		...
		if(stage == "before"):
			...
			if(isinstance(event, str) and event in ["create", "update", "save", "delete"]):
					...
					if(id):
						...
						if(key):
							...
							if(key in self._related):
								if(isinstance(self._related[key], dict) and ("value" in self._related[key])):
									if(("saved" in self._related[key]) and self._related[key]["saved"] == True):
										# return the cached related result
										return self._related[key]["value"]
							
							if(type == "hasOne"):
								if(value):
									result = model.findFirst(sprintf("%s = :%s:" % (to, to)), bind={to: id}, **kwargs)
							elif(type == "hasMany"):
								value = getattr(self, field)
								if(value):
									result = model.find(sprintf("%s = :%s:" % (to, to)), bind={to: id}, **kwargs)
								...
								debug("%s.__getRelated(relationship: %s) Unsupported relationship type: %s!" % (self.__class__.__name__, relationship, type), level=1)
							...
							if(result):
								...
								debug("%s.__getRelated(relationship: %s) No result could not be defined!" % (self.__class__.__name__, relationship), level=1)
						else:
							debug("%s.__getRelated(relationship: %s) The relationship key could not be defined!" % (self.__class__.__name__, relationship), level=1)
					else:
						debug("%s.__getRelated(relationship: %s) The foreign key ID could not be obtained!" % (self.__class__.__name__, relationship), level=1)
				else:
					debug("%s.__getRelated(relationship: %s) The primary key field was not specified!" % (self.__class__.__name__, relationship), level=1)
			else:
				debug("%s.__getRelated(relationship: %s) The model is not a subclass of Model!" % (self.__class__.__name__, relationship), level=1)
		else:
			debug("%s.__getRelated(relationship: %s) The relationship was not defined as a dictionary!" % (self.__class__.__name__, relationship), level=1)
			...
			if(model):
				if(isinstance(value, model)):
					...
					if(id):
						...
						debug("%s.__setRelated(relationship: %s, value: %s) Provided model instance has no valid ID!" % (self.__class__.__name__, relationship, value, model), error=True)
				else:
					debug("%s.__setRelated(relationship: %s, value: %s) Provided model instance is not an instance of %s!" % (self.__class__.__name__, relationship, value, model), error=True)
			else:
	...
	def __syncRelated(self, event=None, stage=None):
		...
		if(isinstance(relationships, dict) and len(relationships) > 0):
			if(isinstance(self._related, dict) and len(self._related) > 0):
				debug("Found %d related items" % (len(self._related)), indent=1, level=2)
	...
	def beforeChange(self, event):
		debug("%s.beforeChange(event: %s) called..." % (self.__class__.__name__, event), level=1)
		return True
	...
	def afterChange(self, event):
		debug("%s.afterChange(event: %s) called..." % (self.__class__.__name__, event), level=1)
		pass
			...
			if(action):
				if(action == "INSERT"):
					...
					if(isinstance(params, dict) and len(params) > 0):
						...
						for attribute in attributes:
							if(attribute in params):
								if(not (attribute in self._snapshot and self._snapshot[attribute] == params[attribute])):
					...
					if(id):
						...
						if(isinstance(kwargs["clause"], str)):
							...
							if(len(clause) > 0):
									...
									def clauseParam(match):
										...
										if(isinstance(params, dict)):
											if(param in params):
												...
												debug("%s.prepareQuery() Unable to find param[%s]!" % (cls.__name__, param), error=True)
												
												valid = False
												
												return ":" + param + ":"
										else:
											debug("%s.prepareQuery() The provided params were invalid! Expected a dictionary, instead found %s" % (cls.__name__, type(params)), error=True)
											
											valid = False
											
											return ":" + param + ":"
									
									clause = re.sub(r"\:([a-z\_]{1}[a-z0-9\_]+)\:", clauseParam, clause, flags=re.IGNORECASE)
								elif(clause.find("?") > 0):
									def clauseParam(match):
										...
										if(isinstance(params, list)):
											if(index < len(params)):
												...
												debug("%s.prepareQuery() Unable to find param[%s]!" % (cls.__name__, index), error=True)
												
												valid = False
												
												return param
										else:
											debug("%s.prepareQuery() The provided params were invalid! Expected a list, instead found %s" % (cls.__name__, type(params)), error=True)
											valid = False
											
											return param
									
									clause = re.sub(r"(?<=\s{1})(\?)", clauseParam, clause, flags=re.IGNORECASE)
								...
								if(valid == False):
									debug("%s.prepareQuery() Failed to replace all clause (%s) placeholders!" % (cls.__name__, clause), error=True)
									
									raise RuntimeError(sprintf("%s.prepareQuery() Failed to replace all clause (%s) placeholders!" % (cls.__name__, clause)))
	...
	def performQuery(cls, query):
		...
		debug("Successfully obtained database service: %s and connection: %s" % (database, connection), level=2)
		
		if(isinstance(query, dict)):
			if("action" in query):
				action = query["action"]
				if(isinstance(action, str) and action in ["SELECT", "INSERT", "UPDATE", "DELETE"]):
					if("string" in query):
						if(isinstance(query["string"], str) and len(query["string"]) > 0):
							if(query["string"].startswith(action)):
								...
								if(cursor):
									try:
										...
										debug(" >>> query.string    = %s" % (string), level=3)
										debug(" >>> cursor.rowcount = %s" % (cursor.rowcount), level=3)
										debug(" >>> cursor type     = %s" % type(cursor), level=3)
										
										if(action == "SELECT"):
											if(("limit" in query) and isinstance(query["limit"], int) and query["limit"] == 1):
										...
										if(action == "SELECT"):
											...
											if(connection.autocommit == True):
												debug("%s.performQuery() Committing changes to database automatically as per configuration..." % (cls.__name__), level=1)
												
												database.commit(connection=connection)
											else:
												debug("%s.performQuery() Not committing changes to database automatically as per configuration..." % (cls.__name__), level=1)
									except Exception as e:
										debug("%s.performQuery() An exception occurred while trying to perform the query!" % (cls.__name__), error=True)
										debug(e)
									...
									debug("%s.performQuery() Failed to obtain database cursor!" % (cls.__name__), error=True)
							else:
								debug("%s.performQuery() The provided query string (%s) does not begin with the expected action: %s!" % (cls.__name__, string, action), error=True)
						else:
							debug("%s.performQuery() The provided query string was not a string, but rather of type: %s!" % (cls.__name__, type(string)), error=True)
					else:
						debug("%s.performQuery() No query string was provided!" % (cls.__name__), error=True)
				else:
					debug("%s.performQuery() The provided query action (%s) was invalid!" % (cls.__name__, action), error=True)
			else:
				debug("%s.performQuery() No query action was provided!" % (cls.__name__), error=True)
		else:
			debug("%s.performQuery() The query was invalid! It should be a dictionary!" % (cls.__name__), error=True)
