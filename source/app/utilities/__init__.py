...
def repeater(delay, method, *args, **kwargs):
    debug(
        "repeat(delay = %s, method = %s, args = %s, kwargs = %s)"
        % (delay, method, args, kwargs)
    )
...
def date(format=None, timestamp=None, **kwargs):
    ...
    if "date" in kwargs and isinstance(kwargs["date"], str):
        if "format_for_input_date" in kwargs and isinstance(
            kwargs["format_for_input_date"], str
        ):
        ...
        try:
            ...
            debug(
                "Exception thrown (%s) while converting date string (%s) with format (%s) to timestamp!"
                % (str(e), kwargs["date"], format_for_input_date),
                error=True,
            )
            return None
