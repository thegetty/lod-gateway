...
class BaseManager(ABC):
    ...
    def getSourceData(self, URL=None, options=None):
        ...
        if not (isinstance(URL, str) and len(URL) > 0):
            debug(
                "%s.getData(URL: %s) The provided URL was invalid!"
        ...
        try:
            ...
            if isinstance(response, requests.models.Response):
                if response.status_code == 200:
                    ...
                    debug(
                        "%s.getData(URL: %s) The response status code was invalid; expected HTTP/1.1 200 OK, instead received %s!"
                        % (self.__class__.__name__, URL, response.status_code),
                        error=True,
                    )
                    # debug(dir(response))
                ...
                debug(
                    "%s.getData(URL: %s) The response (%s) was invalid!"
                    % (self.__class__.__name__, URL, type(response)),
                    error=True,
                )
        except Exception as e:
            debug(
                "%s.getData(URL: %s) The request resulted in an exception being thrown: %s!"
    ...
    def processItem(self, item, **kwargs):
        debug(
            "%s.processItem(item: %s) called..." % (self.__class__.__name__, item),
            level=1,
        )
            ...
            if transformerClass:
                ...
                if entity:
                    if entity.getData():
                        ...
                        if not entitySpace:
                            raise RuntimeError(
                                "Unable to obtain transformer's namespace!"
                            )

                        entityName = entity.getEntityName()
                        if not entityName:
                            raise RuntimeError(
                                "Unable to obtain transformer's entity name!"
                            )

                        entityUUID = entity.getUUID()
                        if not entityUUID:
                            raise RuntimeError(
                                "Unable to obtain transformer's entity UUID!"
                            )
                            ...
                            if data:
                                if options["output"] == "debug":
                                    ...
                                    file = os.path.abspath(
                                        "/usr/local/mart/data/transformer/"
                                        + entitySpace
                                        + "/"
                                        + entityName
                                        + "/"
                                    ...
                                    try:
                                        ...
                                        debug(
                                            "An exception occurred while trying to create the output directory path or write to a file!",
                                            error=True,
                                        )
                                else:
                                    if self.storeEntity(entity, data=data):
                                        debug(
                                            "Successfully saved %s..." % (entity),
                                            level=1,
                                        )
                                        debug(
                                            "This record will be available via the web service at: %s"
                                            % (entity.generateEntityURI()),
                                            indent=1,
                                        )
                                        ...
                                        debug(
                                            "Failed to save %s!" % (entity), error=True
                                        )

                                        response = False
                            else:
                                debug(
                                    "%s.processItem() Unable to serialize data for specified item: %s!"
                                    % (self.__class__.__name__, item),
                                    error=True,
                                )
                        else:
                            debug(
                                "%s.processItem() Unable to map data for specified item: %s!"
                                % (self.__class__.__name__, item),
                                error=True,
                            )
                    else:
                        debug(
                            "%s.processItem() Unable to obtain data for specified item: %s!"
                            % (self.__class__.__name__, item),
                            error=True,
                        )
                else:
                    debug(
                        "%s.processItem() Unable to instantiate transformer class for specified item: %s!"
                        % (self.__class__.__name__, item),
                        error=True,
                    )
            else:
                debug(
                    "%s.processItem() Unable to determine transformer class for specified item: %s!"
                    % (self.__class__.__name__, item),
                    error=True,
                )
        else:
            debug(
                "%s.processItem() Provided item was not a dictionary as expected, but rather of type: %s"
                % (self.__class__.__name__, type(item)),
                error=True,
            )
    ...
    def storeEntity(self, entity, data=None, **kwargs):
        debug(
            "%s.storeEntity(entity: %s, kwargs: %s) called..."
            % (self.__class__.__name__, entity, kwargs),
            level=1,
        )

        if not entity:
            debug(
                "%s.storeEntity() The 'entity' parameter was invalid!"
        ...
        if not namespace:
            debug(
                "%s.storeEntity() The 'namespace' parameter was invalid!"
                % (self.__class__.__name__),
                error=True,
            )
            return False
        ...
        if not entityName:
            debug(
                "%s.storeEntity() The 'entityName' parameter was invalid!"
                % (self.__class__.__name__),
                error=True,
            )
            return False
        ...
        if not entityUUID:
            debug(
                "%s.storeEntity() The 'UUID' parameter was invalid!"
                % (self.__class__.__name__),
                error=True,
            )
            return False
        ...
        if not entityURI:
            entityURI = BaseTransformer.assembleEntityURI(
                namespace=namespace, entityName=entityName, UUID=UUID
            )
        ...
        record = Record.findFirstOrCreateNewInstance(
            "namespace = :namespace: AND entity = :entity: AND uuid = :uuid:",
            bind={"namespace": namespace, "entity": entityName, "uuid": entityUUID},
        )
        if record:
            ...
            if record.save():
                ...
                if GraphStore.update(entityURI, record.data):
                    ...
                    debug(
                        "Failed to update %s within the graph store!" % (record),
                        error=True,
                    )
            ...
            debug(
                "Failed to find or create a Record instance for %s!" % (entity),
                error=True,
            )
    ...
    def getEntityNamespace(self, entity, **kwargs):
        ...
        if "namespace" in kwargs:
            ...
            elif entity.__module__ == "cromulent.model" and getattr(
                entity, "_namespace", None
            ):
...
class ActivityStreamManager(BaseManager):
    ...
    def processSourceData(self, data, response=None):
        debug(
            "%s.processActivityStreams(data: %s, response: %s) called..."
            % (self.__class__.__name__, data, response),
            level=1,
        )
    ...
    def processStreams(self):
        ...
        if isinstance(transformers, dict) and len(transformers) > 0:
                        ...
                        if isinstance(endpoint, str):
                            ...
                            if isinstance(types, list) and len(types) > 0:
                                ...
                                if endpoint in endpoints:
                                    endpoints[endpoint]["transformers"].append(
                                        transformer
                                    )
                                else:
                                    endpoints[endpoint] = {
            ...
            if isinstance(endpoints, dict) and len(endpoints) > 0:
                debug(
                    "Found %d Activity Stream endpoints to process..."
                    % (len(endpoints)),
                    indent=1,
                    level=2,
                )
                ...
                for endpoint in endpoints:
                    ...
                    if isinstance(data, dict):
                        debug(
                            "Processing Activity Stream endpoint: %s for namespace: %s"
                            % (endpoint, namespace),
                            indent=2,
                            level=2,
                        )
                ...
                raise RuntimeError(
                    "Failed to assemble set of Activity Stream endpoints for processing!"
                )
        else:
    ...
    def processStream(
        self, URL=None, namespace=None, transformers=None, options=None, direction=None
    ):
        """Process the registered Activity Streams and convert any referenced items of interest"""

        debug(
            "BaseManager.processActivityStream(URL: %s, namespace: %s, direction: %s) called..."
            % (URL, namespace, direction),
            level=1,
        )
        ...
        if not (isinstance(URL, str) and len(URL) > 0):
            raise RuntimeError(
                sprintf(
                    "Missing Activity Stream URL! Expected a string, found %s instead!"
                    % (type(URL))
                )
            )
        ...
        if not (isinstance(namespace, str) and len(namespace) > 0):
            raise RuntimeError(
                sprintf(
                    "Missing Activity Stream Namespace! Expected a string, found %s instead!"
                    % (type(namespace))
                )
            )
        ...
        if not (isinstance(transformers, list) and len(transformers) > 0):
            raise RuntimeError(
                sprintf(
                    "Missing Activity Stream Transformers! Expected a list, found %s instead!"
                    % (type(transformers))
                )
            )
        ...
        stream = Stream.findFirstOrCreateNewInstance(
            "namespace = :namespace: and base_url = :base_url:",
            bind={"namespace": namespace, "base_url": URL},
        )
        if stream:
                ...
                if isinstance(item, dict):
                    ...
                    if isinstance(itemID, str) and len(itemID) > 0:
                        if isinstance(lastID, str) and (lastID == itemID):
                            debug(
                                "BaseManager.processActivityStream(URL: %s, namespace: %s, direction: %s) The current ID (%s) has been seen before... breaking now..."
                                % (URL, namespace, direction, itemID),
                                level=1,
                            )
                        ...
                        if isinstance(eventType, str) and eventType in eventTypes:
                            ...
                            if isinstance(recordURL, str) and len(recordURL) > 0:
                                ...
                                if isinstance(recordType, str) and len(recordType) > 0:
                                    debug(
                                        "ActivityStream Type: %s (%s)"
                                        % (recordType, recordURL)
                                    )
                                        ...
                                        if isinstance(transformer, dict):
                                            ...
                                            if isinstance(streamTypes, list):
                                                if recordType in streamTypes:
                                                    debug(
                                                        "Found a matching transformer class supporting types: %s for this namespace: %s"
                                                        % (streamTypes, namespace),
                                                        level=2,
                                                    )
                                                    # debug(transformer)
                                                ...
                                                debug(
                                                    "BaseManager.processActivityStream() The current transformer stream types list was invalid! It must be a list instance, but was of type '%s' instead!"
                                                    % (type(streamTypes)),
                                                    error=True,
                                                )
                                        else:  # invalid transformer
                                            debug(
                                                "BaseManager.processActivityStream() The current transformer was invalid! It must be a dictionary (dict) instance, but was of type '%s' instead!"
                                                % (type(transformer)),
                                                error=True,
                                            )
                                    ...
                                    if stream:
                                        ...
                                        if stream.save():
                                            debug(
                                                "BaseManager.processActivityStream() Successfully updated the %s record to note the last processed item ID (%s)..."
                                                % (stream, itemID)
                                            )
                                            ...
                                            debug(
                                                "BaseManager.processActivityStream() Failed to update the %s record to note the last processed item ID (%s)!"
                                                % (stream, itemID),
                                                error=True,
                                            )
                                    ...
                                    debug(
                                        "BaseManager.processActivityStream() The current Activity Stream record type was invalid! It must be a string, but was of type '%s' instead!"
                                        % (type(recordType)),
                                        error=True,
                                    )
                            else:  # invalid recordURL
                                debug(
                                    "BaseManager.processActivityStream() The current Activity Stream record URL was invalid! It must be a string, but was of type '%s' instead!"
                                    % (type(recordURL)),
                                    error=True,
                                )
                        else:  # invalid eventType
                            debug(
                                "BaseManager.processActivityStream() The current Activity Stream event type (%s) was invalid! It must be a string and be one of the following: %s!"
                                % (type(eventType), eventTypes),
                                error=True,
                            )
                    else:  # invalid itemID
                        debug(
                            "BaseManager.processActivityStream() The current Activity Stream item ID (%s) was invalid! It must be a non-empty string!"
                            % (type(itemID)),
                            error=True,
                        )
                else:  # invalid item
                    debug(
                        "BaseManager.processActivityStream() The current Activity Stream item was invalid! It must be a dictionary (dict) instance!",
                        error=True,
                    )
    ...
    def getStreamItems(self, URL=None, options=None, direction=None):
        ...
        debug(
            "BaseManager.getActivityStreamItems(URL: %s, direction: %s) called..."
            % (URL, direction),
            level=1,
        )
        ...
        if not (isinstance(URL, str) and len(URL) > 0):
            raise RuntimeError(
                sprintf(
                    "No valid Activity Stream URL was provided! URL was of type %s instead of being a non-empty string!"
                    % (type(URL))
                )
            )
        ...
        if isinstance(options, dict):
            if "direction" in options:
                if isinstance(options["direction"], str) and options["direction"] in [
                    "first",
                    "current",
                    "last",
                ]:
        ...
        if not (
            isinstance(direction, str) and direction in ["first", "current", "last"]
        ):
            direction = "last"
        ...
        if data:
            ...
            if dataType == "OrderedCollection":
                ...
                if totalItems > 0:
                    ...
                    retry = 0
                    retries = get(
                        options, "retries", os.getenv("ACTIVITY_STREAM_MAX_RETRIES", 3)
                    )
                        ...
                        if pageURL:
                            ...
                            if data:
                                ...
                                if items and len(items) > 0:
                                    ...
                                    debug(
                                        "BaseManager.getActivityStreamItems() No items for %s!"
                                        % (pageURL),
                                        error=True,
                                    )
                                    break
                                ...
                                debug(
                                    "BaseManager.getActivityStreamItems() No data for %s!"
                                    % (pageURL),
                                    error=True,
                                )
                                ...
                                if retry < retries:
                                    sleep(
                                        randint(3, 10)
                                    )  # Sleep for 3 - 10 seconds before retrying
                                    retry += 1
                                else:  # The maximum number of retries has been exceeded, so break here...
                                    debug(
                                        "BaseManager.getActivityStreamItems() Maximum Number of Retries (%d/%d) Reached for Activity Stream Page URL: %s! Breaking Now!"
                                        % ((retry + 1), retries, pageURL),
                                        error=True,
                                    )
                                    break
                        else:  # no valid page URL
                            debug(
                                "BaseManager.getActivityStreamItems() No valid page URL!",
                                error=True,
                            )
                            break
                else:
                    debug(
                        "BaseManager.getActivityStreamItems() No items were found in the OrderedCollection for %s!"
                        % (URL),
                        error=True,
                    )
            else:
                debug(
                    "BaseManager.getActivityStreamItems() The expected response data type of OrderedCollection was not found for %s!"
                    % (URL),
                    error=True,
                )
        else:
            debug(
                "BaseManager.getActivityStreamItems() No data could be obtained for %s!"
                % (URL),
                error=True,
            )
    ...
    def process(self, **kwargs):
        ...
        if "id" in kwargs:
            if not "namespace" in kwargs:
                debug(
                    "When using the manual records manager, one must also specify a valid namespace via the --namespace flag!",
                    error=True,
                )
                exit()
            ...
            if not "entity" in kwargs:
                debug(
                    "When using the manual records manager, one must also specify a valid entity name via the --entity flag!",
                    error=True,
                )
                exit()
    ...
    def getItems(self, **kwargs):
        ...
        if "id" in kwargs:
            ...
            if isinstance(ids, list) and len(ids) > 0:
                ...
                for id in ids:
                    debug(
                        "%s.getItems() ID = %s" % (self.__class__.__name__, id), level=1
                    )
                ...
                debug(
                    "%s.getItems() No IDs provided!" % (self.__class__.__name__),
                    error=True,
                )
        else:
            pass
