                                    ...
                                    if not file.startswith("Base"):
                                        ...
                                        if module:
                                            ...
                                            if klass:
                                                ...
                                                if instance:
                                                    endpointURL = (
                                                        instance.activityStreamEndpoint()
                                                    )
                                                    if (
                                                        isinstance(endpointURL, str)
                                                        and len(endpointURL) > 0
                                                    ):
                                                        transformer["stream"][
                                                            "endpoint"
                                                        ] = endpointURL
                                                    ...
                                                    endpointOptions = (
                                                        instance.activityStreamEndpointOptions()
                                                    )
                                                    if (
                                                        isinstance(
                                                            endpointOptions, dict
                                                        )
                                                        and len(endpointOptions) > 0
                                                    ):
                                                        transformer["stream"][
                                                            "options"
                                                        ] = endpointOptions
                                                    ...
                                                    types = (
                                                        instance.activityStreamObjectTypes()
                                                    )
                                                    if (
                                                        ...
                                                        raise RuntimeError(
                                                            sprintf(
                                                                "Failed to obtain a list of the %s transformer's supported Activity Stream types!"
                                                                % (modName)
                                                            )
                                                        )
                                        else:
...
class BaseTransformer(ABC):
    def __init__(self, *args, id=None, URL=None, **kwargs):
        ...
        debug(
            "%s.__init__(id: %s, URL: %s) called..."
            % (self.__class__.__name__, id, URL),
            level=1,
        )
    ...
    def generateEntityName(self, **kwargs):
        ...
        debug(
            "%s.generateEntityName(%s) called..." % (self.__class__.__name__, kwargs),
            level=1,
        )
    ...
    def generateEntityURI(self, **kwargs):
        ...
        if not (isinstance(namespace, str) and len(namespace) > 0):
            raise RuntimeError(
                "Invalid namespace parameter! It must be a valid, non-empty string!"
            )
        ...
        if not (entity and getattr(entity, "__module__", None) == "cromulent.model"):
            raise RuntimeError(
                "Invalid entity parameter! It must be an instance of a CROM model type!"
            )
        ...
        if not (isinstance(UUID, str) and len(UUID) > 0):
            raise RuntimeError(
                "Invalid UUID parameter! It must be a valid UUIDv4 string representation!"
            )

        return self.assembleEntityURI(
            namespace=namespace, entityName=entityName, UUID=UUID, **kwargs
        )
    ...
    def assembleEntityURI(cls, namespace=None, entityName=None, UUID=None, **kwargs):
        ...
        if not (isinstance(baseURL, str) and len(baseURL) > 0):
            raise RuntimeError(
                "Missing or invalid 'LOD_BASE_URL' environment variable!"
            )
            ...
            if entity:
                ...
                if options["source"]:
                    if options["source"] == True:
                        debug(
                            self.data,
                            format="JSON",
                            label="Debug Output for self.data:",
                        )
                    elif (
                        isinstance(options["source"], str)
                        and len(options["source"]) > 0
                    ):
                        debug(
                            get(self.data, options["source"]),
                            format="JSON",
                            label=sprintf("self.data (%s):" % (options["source"])),
                        )
                ...
                if isinstance(options["method"], str):
                    if not options["method"].startswith("map"):
                        debug(
                            "The specified method (%s.%s) does not begin with 'map'; only map methods are supported for this use-case!"
                            % (self.__class__.__name__, options["method"]),
                            error=True,
                            indent=1,
                        )
                    elif options["method"] not in methodNames:
                        debug(
                            "The specified method (%s.%s) does not exist!"
                            % (self.__class__.__name__, options["method"]),
                            error=True,
                            indent=1,
                        )
    ...
    def createGettyTrustGroup(self):
        ...
        JPGT.classified_as = Type(
            ident="http://vocab.getty.edu/ulan/500000003", label="Corporate Bodies"
        )
