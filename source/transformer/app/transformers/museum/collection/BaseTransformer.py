...
class BaseTransformer(SharedMuseumBaseTransformer):
    def activityStreamEndpoint(self):
        ...
        if isinstance(baseURL, str) and len(baseURL) > 0:
            ...
            raise RuntimeError(
                "Unable to obtain DOR_BASE_URL environment variable! Please check runtime environment configuration!"
            )
    ...
    def assembleHeaders(self):
        ...
        if apiUser:
            if apiKey:
                ...
                if apiVersion:
                    headers["Accept"] = (
                        "application/json;charset=UTF-8;version=" + apiVersion
                    )
                else:
        ...
        if self.data:
            if "uuid" in self.data:
                if isinstance(self.data["uuid"], str):
                    ...
                    debug(
                        "The current record's UUID was not a string, but of type '%s' instead!"
                        % (type(self.data["uuid"])),
                        error=True,
                    )
            else:
    ...
    def getData(self):
        ...
        if isURL(URL):
            debug(
                "%s.getData(%s) Attempt to Obtain URL..."
            ...
            if isinstance(headers, dict):
                ...
                while True:
                    try:
                        ...
                        if isinstance(response, requests.models.Response):
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        if isinstance(data, dict):
                                            ...
                                            debug(
                                                "%s.getData(%s) Invalid JSON Data!"
                                                % (self.__class__.__name__, URL),
                                                error=True,
                                            )
                                    else:
                                        debug(
                                            "%s.getData(%s) Invalid JSON String!"
                                            % (self.__class__.__name__, URL),
                                            error=True,
                                        )
                                else:
                                    debug(
                                        "%s.getData(%s) Invalid Response Text!"
                                        % (self.__class__.__name__, URL),
                                        error=True,
                                    )
                            else:
                                debug(
                                    "%s.getData(%s) Invalid HTTP Response Status Code! Expected HTTP/1.1 200 OK; Received %d!"
                                    % (
                            ...
                            debug(
                                "%s.getData(%s) Invalid HTTP Response!"
                                % (self.__class__.__name__, URL),
                                error=True,
                            )
                        ...
                        if retry < retries:
                            debug(
                                "%s.getData(%s) HTTP Request Failed! Retry Attempt %d/%d..."
                                % (self.__class__.__name__, URL, (retry + 1), retries),
                                error=True,
                            )
                            ...
                            debug(
                                "%s.getData(%s) Maximum Number of Retries (%d/%d) Reached for HTTP Request! Breaking Now!"
                                % (self.__class__.__name__, URL, (retry + 1), retries),
                                error=True,
                            )
                            break
                        ...
                        debug(
                            "%s.getData(%s) HTTP Request Raised Exception: %s!"
                ...
                debug(
                    "%s.getData(%s) Missing HTTP Request Headers!"
                    % (self.__class__.__name__, URL),
                    error=True,
                )
        else:
            debug(
                "%s.getData(%s) Invalid URL!" % (self.__class__.__name__, URL),
                error=True,
            )
