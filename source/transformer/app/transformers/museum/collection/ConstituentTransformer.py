    ...
    def mapBirthPlace(self, entity, data):
        if get(data, "type") == "INDIVIDUAL":
            if has(data, "display.places.birth"):
                ...
                date_birth_began = get(data, "display.places.birth.date.began.iso")
                date_birth_ended = get(data, "display.places.birth.date.ended.iso")
    ...
    def mapDeathPlace(self, entity, data):
        if get(data, "type") == "INDIVIDUAL":
            if has(data, "display.places.death"):
                ...
                date_death_began = get(data, "display.places.death.date.began.iso")
                date_death_ended = get(data, "display.places.death.date.ended.iso")
        ...
        if nationality:
            ...
            if classification:
                ...
                type.classified_as = Type(
                    ident="http://vocab.getty.edu/aat/300379842", label="Nationality"
                )
