...
class ExhibitionTransformer(BaseTransformer):
    ...
    def mapClassification(self, entity, data):
        entity.classified_as = Type(
            ident="http://vocab.getty.edu/aat/300417531", label="Exhibition"
        )
    ...
    def mapTitle(self, entity, data):
        ...
        if title:
            name = Name(
                ident=self.generateEntityURI(sub=["name"]), label="Display Title"
            )
            ...
            for curator in curators:
                person = Person(
                    ident=self.generateEntityURI(
                        entity=Person, id=get(curator, "uuid")
                    ),
                    label=get(curator, "display.value"),
                )
                person.classified_as = Type(
                    ident="http://vocab.getty.edu/aat/300025633", label="Curators"
                )
                person.classified_as = Type(
                    ident="http://vocab.getty.edu/aat/300417893",
                    label="Exhibition Curators",
                )
        ...
        if isinstance(venues, list) and len(venues) > 0:
            ...
            if isinstance(events, list) and len(events) > 0:
                for event in events:
                    ...
                    if (
                        ...
                        and event["subtype"] == "TRAVELLING"
                        and event["primary"] == True
                    ):
                        travelling = Activity(
                            ident=self.generateEntityURI(
                                entity=Activity, UUID=get(event, "uuid")
                            )
                        )

                        travelling._namespace = self.getNamespace()
                        travelling._uuid = get(event, "uuid")
                        travelling._label = sprintf(
                            "Travelling Exhibition for %s"
                            % (
                                get(
                                    data,
                                    "display.title.display.value",
                                    default=get(event, "uuid"),
                                )
                            )
                        )

                        travelling.classified_as = Type(
                            ident="http://vocab.getty.edu/aat/300054766",
                            label="Exhibitions (Events)",
                        )

                        travelling.classified_as = Type(
                            ident="http://vocab.getty.edu/aat/300054773",
                            label="Travelling Exhibitions",
                        )
            ...
            if travelling:
                ...
                if dates:
                    timespan = TimeSpan(
                        ident=self.generateEntityURI(
                            entity=Activity, UUID=get(event, "uuid"), sub=["timespan"]
                        )
                    )
            ...
            for venue in venues:
                ...
                place = Place(
                    ident=self.generateEntityURI(entity=Place, UUID=get(venue, "uuid")),
                    label=get(venue, "display.value"),
                )
                ...
                if isinstance(classifications, list) and len(classifications) > 0:
                    for classification in classifications:
                        place.classified_as = Type(
                            ident=get(classified, "id"), label=get(classified, "label")
                        )
                ...
                if isinstance(organizers, list) and len(organizers) > 0:
                    for organizer in organizers:
                        ...
                        if get(organizer, "type") == "INDIVIDUAL":
                            actor = Person(
                                ident=self.generateEntityURI(
                                    entity=Person, UUID=get(organizer, "uuid")
                                )
                            )
                        else:
                            actor = Group(
                                ident=self.generateEntityURI(
                                    entity=Group, UUID=get(organizer, "uuid")
                                )
                            )
                ...
                if isinstance(objects, list) and len(objects) > 0:
                    ...
                    for object in objects:
                        ...
                        artifact = Object(
                            ident=self.generateEntityURI(
                                entity=Object,
                                UUID=get(object, "uuid"),
                                entityName="object",
                            ),
                            label=get(object, "display.value"),
                        )
                        ...
                        if (
                            ...
                            for classification in classifications:
                                artifact.classified_as = Type(
                                    ident=get(classified, "id"),
                                    label=get(classified, "label"),
                                )
                ...
                if manager.storeEntity(activity):
                    if travelling:
                        part = Activity(
                            ident=self.generateEntityURI(
                                entity=Activity, UUID=get(venue, "uuid")
                            ),
                            label=None,
                        )
                        ...
                        entity.motivated = Activity(
                            ident=activity.id, label=activity._label
                        )
                else:
                    debug(
                        "%s.mapActivities() Failed to store Activity (%s) entity for Exhibition (%s) entity!"
                        % (self.__class__.__name__, activity, entity),
                        error=True,
                    )
            ...
            if travelling:
                if manager.storeEntity(travelling):
                    entity.motivated = Activity(
                        ident=travelling.id, label=travelling._label
                    )
                else:
                    debug(
                        "%s.mapActivities() Failed to store Activity (%s) entity for Exhibition (%s) entity!"
