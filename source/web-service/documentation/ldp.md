...
The POST endpoint handles ID assignment in the following order:

1. **Slug header**: If provided, the Slug value becomes the top-level ID (replacing any existing ID)
2. **Existing @id/id**: If present and valid, the ID is rebased relative to the destination container
3. **Auto-generated UUID**: If no ID is present, a UUID is generated and assigned
...
The LDP PUT endpoint allows clients to create or update resources at a specific URI. Unlike POST, PUT requires the resource to have an ID that matches the destination URI.

### PUT Behavior

- **ID Injection**: If the request body has no `@id` or `id` field, the destination URI is injected
- **ID Matching**: The body ID (if present) must match the destination URI (after normalization)
- **Update vs Create**: Returns 200 OK if updating an existing record, 201 Created if creating a new one
- **Response**: Returns the full record representation (like GET), with prefixed IDs

### PUT ID Handling

The PUT endpoint handles ID assignment as follows:

1. **Missing ID**: If no `@id`/`id` in body, the destination URI is injected
2. **ID Matching**: The body ID must match the destination URI after normalization:
   - Strip `idPrefix` from both IDs
   - Strip leading slashes
   - Compare the normalized IDs
3. **ID Mismatch**: Returns 422 Unprocessable Entity if IDs don't match
...
```http
PUT /my-namespace/my-resource HTTP/1.1
Authorization: Bearer <token>
Content-Type: application/ld+json

{
  "@id": "my-namespace/my-resource",
  "@type": "Object",
  "name": "My Resource"
}
```
...
```json
{
  "@id": "https://data.getty.edu/my-namespace/my-resource",
  "@type": "Object",
  "name": "My Resource"
}
```
...
Headers:
```
Location: https://data.getty.edu/my-namespace/my-resource
Content-Type: application/ld+json
Link: <https://data.getty.edu/my-namespace/my-resource>; rel="canonical"; type="application/ld+json"
```

#### PUT without ID (ID is injected)
...
The server injects the destination URI as the `@id`, resulting in the same behavior as above.

#### PUT with relative ID
...
The relative ID `my-resource` is normalized and compared to the destination. If the namespace is `my-namespace`, the normalized IDs match.
...
#### PUT with absolute ID (different base)
...
{
  "@id": "https://other-host.com/my-namespace/my-resource",
  "@type": "Object",
  "name": "My Resource"
}
```

The ID is normalized by stripping the prefix, resulting in `my-namespace/my-resource`, which matches the destination.

#### PUT to a nested container (requires LDP_AUTOCREATE_CONTAINERS)

```http
PUT /my-namespace/parent/child/resource HTTP/1.1
Authorization: Bearer <token>
Content-Type: application/ld+json

{
  "@id": "my-namespace/parent/child/resource",
  "@type": "Object",
  "name": "Nested Resource"
}
```

If `LDP_AUTOCREATE_CONTAINERS` is enabled, the server creates the `/my-namespace/parent/child/` container automatically. Otherwise, returns 422.
...
| 422 Unprocessable Entity | Invalid JSON-LD, ID mismatch, or missing parent container |

### PUT Response Headers

All PUT responses include:
- `Location`: The URI of the created/updated resource
- `Content-Type`: `application/ld+json`
- `Link`: Canonical link to the resource
- `Last-Modified`: Timestamp of the update
...
### ID Normalization and Content Negotiation

The PUT endpoint normalizes IDs by stripping the `idPrefix` and leading slashes before comparison. This allows:
- Relative IDs (e.g., `my-resource`)
- Absolute IDs with different bases (e.g., `https://other-host.com/...`)
- IDs with or without leading slashes

However, this normalization may cause issues if the same resource is accessible via multiple URIs. Clients should be aware that the server may rewrite relative IDs to match the destination URI.

## API Summary

| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/{container}/` | Get container (paginated) |
| GET | `/{entity_id}` | Get record by ID |
| POST | `/{container}/` | Create resource in container |
| PUT | `/{entity_id}` | Create or update resource at URI |
| DELETE | `/{entity_id}` | Delete resource |
