...
def update_item(entity_id):
    ...
    #    - if Resource:
    #      - First, should fail if item exists at where a Slug header would put it (PUT is necessary) or if Container exists there
    ...
    cid = f"/{entity_id}"
    # Is there a container here?
    if c := get_container(cid, optimistic=True):
        posted_representation = parse_representation(request)
