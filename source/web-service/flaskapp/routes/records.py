...
def update_item(entity_id):
    ...
    #      - relative ids ON THE CONTAINER (eg /object/1/2 into container /object/1/ would be "2" MUST have @base)
    ...
    c = get_container(cid, optimistic=True)

    if record and record.data:
        # there is a record here, and the JSON-LD validated.
        if posted_representation.is_basic_container():
            # Cannot create a container at same URI as a Record
            current_app.logger.error(
                f"Request failed - POSTed resource is a ldp:BasicContainer, but there is already a non-Container Resource at this URI {entity_id}"
            )
            response = construct_error_response(
                status_nt(
                    409,
                    "Conflict Error",
                    "Cannot create a BasicContainer at the same URI as an existing resource",
                )
            )
            abort(response)
    ...
    if c := get_container(cid, optimistic=True):
        ...
        if len(container_breadcrumbs) > 1:
            if parent := get_container(container_breadcrumbs[-2], optimistic=True):
                # Add to existing container
                pass
            else:
                # If LDP_AUTOCREATE_CONTAINERS is True, create what is needed, and add the resource to that
                pass
