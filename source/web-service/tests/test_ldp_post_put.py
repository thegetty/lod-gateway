...
class TestPostToDeletedRecords:
    ...
    def test_post_to_deleted_record_path_succeeds(
        ...
        """
        # Create a record first, then delete it
        entity_id = str(uuid4())
        original_data = {
        ...
        record = test_db.session.query(Record).filter_by(entity_id=entity_id).one()
        record.data = None
        ...
        new_record = test_db.session.get(Record, record.id)
    ...
    def test_post_to_active_record_fails_with_409(
        ...
        """
        # Create an active record (not deleted) via POST
        entity_id = str(uuid4())
        original_data = {
            "@id": f"{namespace}/{entity_id}",
            "type": "Object",
    ...
    def test_post_to_deleted_record_with_pagination(
        ...
        """
        # Create multiple records via POST
        ...
        deleted_ids = [created_ids[1], created_ids[3]]
        for eid in deleted_ids:
            record = test_db.session.query(Record).filter_by(entity_id=eid).one()
            record.data = None
            record.datetime_deleted = datetime.now(timezone.utc)
            test_db.session.add(record)
        test_db.session.commit()
        ...
        deleted_entity_id = deleted_ids[0]
        new_data = {
            "@id": f"{namespace}/{deleted_entity_id}",
            "type": "Object",
            "name": "Replacement Resource",
        }

        response = client_ldpapi.post(
            f"{namespace}/{deleted_entity_id}",
            json=new_data,
            content_type="application/ld+json",
            headers={"Authorization": f"Bearer {auth_token}"},
        )
    ...
    def test_post_to_deleted_record_preserves_activity_stream(
        ...
        # Create and delete a record via POST
        entity_id = str(uuid4())
        original_data = {
            "@id": f"{namespace}/{entity_id}",
            "type": "Object",
...
class TestPutEndpoint:
    ...
    def test_put_with_valid_data_updates_existing_record(
        ...
        # Verify update
        updated_record = (
            test_db.session.query(Record).filter_by(entity_id=entity_id).one()
        )
    ...
    def test_put_with_id_field_instead_of_at_id(
        ...
        # Verify record was created
        new_record = test_db.session.query(Record).filter_by(entity_id=entity_id).one()
